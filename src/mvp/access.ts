...
export function buildContexts(db:DB):AppContext[]{
 ...
 return [
  {id:'self',label:'شخصی — اثرگذاری من',roleTitle:'کارمند',scope:`پرونده شخصی ${self.name}`,engineRole:'Employee',personId:self.id,nav:[item('Home','خانه'),item('Expectations','انتظارات و اهداف'),item('Appeals','اعتراض و پیگیری')]},
  ...managerContexts(db),
