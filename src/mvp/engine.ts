...
export type WorkOrigin='Task manager'|'KPI'|'OKR'|'Resolution';
export interface WorkItem{id:string;team:string;workType:string;archetype:Archetype;origin:WorkOrigin;complexity:{factors:number[];total:number;band:string;workUnits:number};roles:Record<WorkRole,string|null>;exception:null|{type:string;severity:'Minor'|'Major'|'Critical'};attributionCause:string|null;events:CanonicalEvent[];}
...
export interface EvaluationPeriod{id:string;cadence:'Quarterly'|'Semiannual';state:PeriodState;lockedAt?:string;publishedAt?:string;}
...
export const avg=(n:number[])=>n.length?n.reduce((a,b)=>a+b,0)/n.length:0;
// The same calculated-dimension formula calculate() falls back to, exposed so exploratory
// views (e.g. a role/team-filtered slice of evidence) can recompute it honestly for a subset
// instead of inventing a different metric.
export function dimsFromEvidence(subset:Evidence[],expected:number):(number|null)[]{
 const verified=subset.filter(e=>e.status==='Verified'),eligible=verified.filter(e=>!e.excluded);
 const completed=verified.reduce((s,e)=>s+units(e.factors)*e.share/100,0);
 return [verified.length?Math.min(100,completed/expected*100):null,verified.length?avg(verified.map(e=>e.quality)):null,eligible.length?avg(eligible.map(e=>e.onTime?100:0)):null,verified.length?avg(verified.map(e=>e.stewardship)):null,verified.length?avg(verified.map(e=>e.collective)):null];
}
...
export function seed():DB{
 ...
 const workItems:WorkItem[]=mock.workItems.map(w=>({id:w.id,team:teamMap.get(w.teamId)||w.teamId,workType:w.workType,archetype:archetype(w.archetype),origin:(w.origin as WorkOrigin)||'Task manager',complexity:{factors:Object.values(w.complexity.factors),total:w.complexity.total,band:w.complexity.band,workUnits:w.complexity.workUnits},roles:w.roleAssignments as Record<WorkRole,string|null>,exception:w.exception as WorkItem['exception'],attributionCause:w.attributionCause,events:normalizeEvents(w.events)}));
 const evidence:Evidence[]=mock.evidenceLedger.map(ev=>{const item=itemMap.get(ev.workItemId)!;const evPeriod=(ev as {period?:string}).period||'2026-08';const current=mock.people.find(p=>p.id===item.roleAssignments.executor)?.scores.periods.find(p=>p.period===evPeriod)||mock.people.find(p=>p.id===item.roleAssignments.executor)?.scores.periods.find(p=>p.period==='2026-08');const exception=item.exception;return {id:ev.id,workItemId:item.id,personId:item.roleAssignments.executor,project:teamMap.get(item.teamId)||item.teamId,title:item.workType,source:ev.sourceSystem,reference:ev.artifactReference,period:evPeriod,factors:Object.values(item.complexity.factors),share:item.roleAssignments.contributor===item.roleAssignments.executor?100:70,quality:current?.dimensions.quality||75,onTime:!exception||exception.type!=='Missed deadline',excluded:['External outage or unavailable dependency','Unrealistic deadline at assignment','Management-created overload'].includes(item.attributionCause||''),stewardship:current?.dimensions.stewardship||75,collective:current?.dimensions.collective||75,status:ev.verificationState==='Verified'?'Verified':ev.verificationState==='Rejected'?'Returned':'Pending',critical:exception?.severity==='Critical',exceptionType:exception?.type,exceptionSeverity:exception?.severity as Evidence['exceptionSeverity'],attributionCause:item.attributionCause,actor:item.roleAssignments.requester}});
