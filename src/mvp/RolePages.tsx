...
export const customPages=new Set(['Home','Expectations','Team outcomes','System quality','Project snapshot','Attribution','Job families','Performance profiles','Integrations','Engine ledger','Calibration facilitator','Quality samples','Governance cases','Payroll preview','Access management']);
...
export default function RolePages({db,context,page,period,navigate}:Props){
 ...
 if(page==='System quality'){const items=context.managerId?db.workItems.filter(w=>scopeIds.has(w.roles.executor||'')):db.workItems;return <Page title="کیفیت سیستم کاری" text="این نما مسئله‌های ناشی از مدیریت، ظرفیت و وابستگی را از عملکرد فرد جدا می‌کند."><Table heads={['کار','علت','شدت','مسئول پاسخ‌گویی','اثر بر مجری']} rows={items.filter(w=>w.attributionCause).map(w=>[tr(w.workType),tr(w.attributionCause),tr(w.exception?.severity),w.attributionCause?.includes('Management')?'مدیر/واگذارکننده':'مالک وابستگی',w.attributionCause==='Employee delay without blocker'?'محاسبه می‌شود':'حذف از قابلیت اتکا'])}/></Page>}
