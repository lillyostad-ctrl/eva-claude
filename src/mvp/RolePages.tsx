...
function SelfHome({db,person,period,navigate}:{db:DB;person:DB['people'][number];period:string;navigate:(p:string)=>void}){
 ...
 return <>
  ...
  <h3>شواهد پشتیبان</h3>
  <section className="panel">{result.all.map(e=><button className="project-item" key={e.id} onClick={()=>navigate('Expectations')}><span>{tr(e.title)}<small>{e.reference} · {tr(e.status)}{e.excluded?' · تأخیر بیرونی حذف شده':''}</small></span><ArrowUpLeft size={16}/></button>)}{!result.all.length&&<div className="empty">برای این دوره شاهدی وجود ندارد.</div>}</section>
...
function ExpectationsPage({db,person,period,save,notify}:{db:DB;person:DB['people'][number];period:string;save:Save;notify:(s:string)=>void}){
 ...
 return <>
  <div className="role-hero"><div><span className="eyebrow">کارمند</span><h1>انتظارات و اهداف</h1><p>ساختار شکست اهداف تیم تا سطح تسک، همه تسک‌هایی که در آن‌ها نقشی دارید، و دفتر کامل شواهد شما — در یک فضای کاری.</p></div><span className="secure-pill"><Target size={15}/>هدف‌محور</span></div>
