...
export default function App(){
 ...
 return <div className="shell" dir="rtl">
  ...
  <main>{page!=='Home'&&<div className="page-heading"><div><div className="eyebrow">{context.roleTitle}</div><h1>{pageLabel}</h1><p>{context.scope} · استاندارد {db.standard}</p></div><Badge>{faPeriod(period)}</Badge></div>}
   {customPages.has(page)&&<RolePages db={db} context={context} page={page} period={period} navigate={navigate}/>}
   ...
   <Workspace db={db} role={role} context={context} period={period} save={save} notify={notify} page={page} selected={selected} setSelected={setSelected}/><footer>ایوا · ارزیابی مبتنی بر شواهد <span>{faNum(db.people.length)} نفر، {faNum(db.workItems.length)} کار و {faNum(db.evidence.length)} رکورد شواهد</span></footer>
